...
useEffect(() => { 
  Api.get(`invoice/invoices/${props.match.params.id}/${props.match.params.access_token}/`)
.then(response => setInvoiceInfo(response.data)).finally(() => {
setLoading(false);
});
...
const downloadPdf = () => {
  Api.get(`invoice/invoices/pdf/${props.match.params.id}/${props.match.params.access_token}/`, {responseType: 'blob'})
...
return (
  ...
  <div className="company col-5">
    <img src={Media_URL.meidaURL + invoiceInfo[0].company.logo} />
  </div>
