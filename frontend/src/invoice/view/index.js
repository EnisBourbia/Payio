...
import { Api } from '../../Api'
import {Menu, MenuItem, Tabs, Tab} from '@mui/material';
...
const fetchInvoices = () => {
    Api.get('/invoice/invoices/?status=Created').then(response => {
      editedPrompt()
    ...
    Api.get('invoice/invoices/?status=Attested&status=Sent&status=Completed').then(response => {
      setAttestedInvoices(response.data)
      setLoading(false);
...
function statusCode(invoice_status) {
  ...
  const downloadPdf = (id, token, company) => {
        Api.get(`invoice/invoices/pdf/${id}/${token}/`, {responseType: 'blob'})
...
return (
    <>
      ...
      >
        <MenuItem component={Link} to="/skapa-faktura" onClick={() => setAnchorEl(null)}>Faktura</MenuItem>
        <MenuItem component={Link} to="/skapa-avtalsfaktura" onClick={() => setAnchorEl(null)}>Avtalsfaktura</MenuItem>
        <MenuItem component={Link} to="/skapa-massfaktura" onClick={() => setAnchorEl(null)}>Massfaktura</MenuItem>
      </Menu>
    </div>
    ...
    <div className="invoice-table">
        ...
        <Tabs value={tabValue} onChange={(e, v) => {setTabValue(v)}} aria-label="basic tabs example">
          <Tab label={`Pågående ${rows.length}`} {...a11yProps(0)} />
          <Tab label={`Attesterade ${rowsAttested.length}`} {...a11yProps(1)} />
        </Tabs>
      </Box>
