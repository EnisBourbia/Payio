...
const [dealInterval, setDealInterval] = useState('Monthly');
const [nextInvoiceDate, setNextInvoiceDate] = useState();
const [nextInvoiceSpecification, setNextInvoiceSpecification] = useState({
    day: 1,
    interval: 1,
    month: 0
})
...
useEffect(() => {
    calculateNextInvoice(dealInterval)
    setCurrentInvoice({...currentInvoice, next_invoice: nextInvoiceDate})
    console.log(2)
},[dealInterval, nextInvoiceSpecification, nextInvoiceDate])
...
useEffect(() => { 
    setCurrentInvoice({...currentInvoice, deal_period: dealInterval})
},[dealInterval])
...
const calculateNextInvoice = (period) => {
    switch(period) {
        case "Monthly":
            let currentDate = new Date()
            let upcommingInvoice = currentDate.setDate(nextInvoiceSpecification.day)
            if (currentDate.getDate() < nextInvoiceSpecification.day || currentInvoice.activation_date.getDate() > currentDate.getDate()) {
                upcommingInvoice = currentDate.setMonth(currentInvoice.activation_date.getMonth()+1)
            } else {
                upcommingInvoice = currentDate.setMonth(currentInvoice.activation_date.getMonth())
            }
            
            setNextInvoiceDate(new Date(currentDate).toLocaleDateString("sv-SE"))
            setCurrentInvoice({...currentInvoice, next_invoice: currentDate})
            break;
        case "Quarterly":
            let currentDateQuarterly = new Date()
            currentDateQuarterly.setDate(nextInvoiceSpecification.day)

            if (currentInvoice.activation_date.getMonth() % 3 === 0) {
                if (currentInvoice.activation_date.getDate() > nextInvoiceSpecification.day) {
                    currentDateQuarterly.setMonth(currentInvoice.activation_date.getMonth()+(nextInvoiceSpecification.month)+(3-(currentInvoice.activation_date.getMonth() % 3)))     
                } else {
                    currentDateQuarterly.setMonth(currentInvoice.activation_date.getMonth()+(nextInvoiceSpecification.month))    
                }
            } else {
                if(currentInvoice.activation_date.getMonth() > 9) {
                    currentDateQuarterly.setMonth(0+nextInvoiceSpecification.month)
                } else {
                    console.log(3-(currentInvoice.activation_date.getMonth() % 3))
                    currentDateQuarterly.setMonth(currentInvoice.activation_date.getMonth()+(nextInvoiceSpecification.month)+(3-(currentInvoice.activation_date.getMonth() % 3)))     
                }
            }
            setNextInvoiceDate(new Date(currentDateQuarterly).toLocaleDateString("sv-SE"))
            setCurrentInvoice({...currentInvoice, next_invoice: new Date(currentDateQuarterly)})
            break;
        case "Yearly":
            let currentDateYearly = new Date()
            currentDateYearly.setDate(nextInvoiceSpecification.day)
            currentDateYearly.setMonth(nextInvoiceSpecification.month)
            if (currentDateYearly < currentInvoice.activation_date) {
                if (currentDateYearly.getMonth() < currentInvoice.activation_date.getMonth() || currentDateYearly.getDate() < currentInvoice.activation_date.getDate()) {
                    currentDateYearly.setFullYear(currentInvoice.activation_date.getFullYear()+1)
                } else {
                    currentDateYearly.setFullYear(currentInvoice.activation_date.getFullYear())
                }
            }
            setNextInvoiceDate(new Date(currentDateYearly).toLocaleDateString("sv-SE"))
            setCurrentInvoice({...currentInvoice, next_invoice: new Date(currentDateYearly)})
            break;
...
return (
    ...
    <Breadcrumbs separator="›" aria-label="breadcrumb">
        ...
        <Typography color="text.primary">Ny avtalsfaktura</Typography>
    </Breadcrumbs>
        <h2>Skapa ny avtalsfaktura</h2>
    ...
    <Grid item xs={12} md={12} lg={6}>
    <span>Skapa fakturor</span><br/>
    <ToggleButtonGroup
      value={dealInterval}
      color="secondary"
      exclusive
      onChange={(e, value) =>  value === null ? null: setDealInterval(value)}
      aria-label="text alignment"
    >   
        <ToggleButton size="small" value="Monthly" onClick={() => calculateNextInvoice('Monthly')} aria-label="left aligned">
            Månatligen
        </ToggleButton>
        <ToggleButton size="small" value="Quarterly" onClick={() => calculateNextInvoice('Quarterly')} aria-label="left aligned">
            Kvartalsvis
        </ToggleButton>
        <ToggleButton size="small" value="Yearly" onClick={() => calculateNextInvoice('Yearly')} aria-label="left aligned">
            Årligen
        </ToggleButton>
    </ToggleButtonGroup>
    { dealInterval === 'Monthly' ?
    <>
    <span> den </span>
    <TextField
        onChange={(e) => {setNextInvoiceSpecification({...nextInvoiceSpecification, day: e.target.value}); setCurrentInvoice({...currentInvoice, period_day: e.target.value})}}
        value={nextInvoiceSpecification.day}
        hiddenLabel
        className="deal-interval-input"
        variant="filled"
        size=""
      />    
    <span> var </span>
    <TextField
        onChange={(e) => {setNextInvoiceSpecification({...nextInvoiceSpecification, interval: e.target.value}); calculateNextInvoice('monthly'); setCurrentInvoice({...currentInvoice, period_interval: e.target.value})}}
        value={nextInvoiceSpecification.interval}
        hiddenLabel
        className="deal-interval-input"
        variant="filled"
        size=""
      ...
      <span> månad </span>
    <br/>
    <span><i>Nästa faktura kommer skapas {nextInvoiceDate}</i></span>
    </>
    :null }
    { dealInterval === 'Quarterly' ?
    <>
    <span> den </span>
    <TextField
        onChange={(e) => {setNextInvoiceSpecification({...nextInvoiceSpecification, day: e.target.value}); setCurrentInvoice({...currentInvoice, period_day: e.target.value})}}
      ...
      />    
    <Select
        className="month-picker-quarterly"
        label={false}
        onChange={(e) => {setNextInvoiceSpecification({...nextInvoiceSpecification, month: e.target.value}); setCurrentInvoice({...currentInvoice, period_month: e.target.value})}}
        defaultValue={nextInvoiceSpecification.month}
        >
            <MenuItem value={0}>Första månaden</MenuItem>
            <MenuItem value={1}>Andra månaden</MenuItem>
            <MenuItem value={2}>Tredje månaden</MenuItem>
        </Select>
    <span> var </span>
    <TextField
        onChange={(e) => {setNextInvoiceSpecification({...nextInvoiceSpecification, interval: e.target.value}); setCurrentInvoice({...currentInvoice, period_interval: e.target.value})}}
        value={nextInvoiceSpecification.interval}
        hiddenLabel
        className="deal-interval-input"
        variant="filled"
    ...
    <Select
        ...
        >
            ...
            <MenuItem value={7}>Augusti</MenuItem>
    ...
    <LocalizationProvider dateAdapter={AdapterDateFns} locale={sv}>
        <DatePicker
            ...
            minDate={new Date()}
            value={currentInvoice.activation_date}
            onChange={(newValue) => {
                setCurrentInvoice({...currentInvoice, activation_date: newValue})
            }}
    ...
    <br/>
    <br/>
    {/* <FormControlLabel control={<Checkbox checked={currentInvoice.auto_attest} onChange={(e) => {setCurrentInvoice({...currentInvoice, auto_attest: e.target.checked})}}/>} label="Attestera skapad faktura automatiskt" /> */}
    <Box>
