...
class InvoiceView(APIView):
    ...
    def patch(self, request):
        for row in request.data[1]:
            ...
            HTML(string=rendered_string).write_pdf('invoice/pdfs/tmp/{}.pdf'.format(invoice_serializer.data['id']), stylesheets=[('invoice/templates/index.css')])
            invoice.save()
        ...
        if invoice_serializer.is_valid():
            ...
            for row in request.data[1]:
                ...
                if article_serializer.is_valid():
                    ...
                    Invoice.objects.get(id=saved_invoice.pk).delete()
            ...
            auto_business_event = BusinessEvent.objects.create(company=Company.objects.get(user=request.user), calendar=CompanyCalendar.objects.get(company=Company.objects.get(user=request.user)), event_type="Verifikation").save()
    ...
    def get(self, request, format=None):
        invoice_status = self.request.query_params.getlist('status')
        invoice_data = Invoice.objects.filter(company = Company.objects.get(user=request.user), status__in=invoice_status).order_by('-invoice_number', 'id')
        serializer = InvoiceSerializer(invoice_data, many=True)
        return Response(serializer.data, status=status.HTTP_200_OK)

    def delete(self, request):
        for invoice in request.data['id']:
...
class InvoicePdf(APIView):
    ...
    def get(self, request, id, access_token):
        ...
        path_to_file =  'invoice/pdfs/tmp/{}.pdf'.format(id)
